...
public class Group {
	...
	public void add(Student stud) throws MyException {
		boolean isEmpty = true;						//assume group is empty
		for (Student st : groupArray) {				//checking each entry for non-null value
			if (st != null) {					
				isEmpty = false;					//group is not empty
				break;
			}
		}
		if (isEmpty) {								//is the group really empty?
			groupArray[0] = stud;					//group is empty, so just add first student
		} else {
			for (int i = 0; i < groupArray.length; i++) {		//group is not empty	
				if (groupArray[i] == null) {			//can add only to empty (null ) cell
					for (Student st : groupArray) {		//check if this student is already in group
					...
					groupArray[i] = stud;				//student not in group, adding to empty cell
					return;
				}
			}
			throw new MyException("Group is full, cannot add!");		//full group, cannot add new student
		}
	}
